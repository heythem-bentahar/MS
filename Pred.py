...
def predcit_page():
        ...
        with col1:
                ...
                S_fourmillements = float(st.checkbox("Symptomes: Fourmillements"))
                S_trouble_de_léquilibre = float(st.checkbox("Symptomes: Trouble de l'équilibre"))
                S_baisse_de_lacuité_visuelle_de_loeil_droit = float(st.checkbox("Symptomes: Baisse de l'Acuité Visuelle de l'Oeil Droit"))
                S_baisse_de_lacuité_visuelle_de_loeil_gauche = float(st.checkbox("Symptomes: Baisse de l'Acuité Visuelle de l'Oeil Gauche"))
                S_faiblesse_musculaire_de_lhémicorps_droit = float(st.checkbox("Symptomes: Faiblesse Musculaire de l'Hémicorps Droit"))
                S_faiblesse_musculaire_de_lémicorps_gauche = float(st.checkbox("Symptomes: Faiblesse Musculaire de l'Emicorps Gauche"))
                S_perte_de_la_sensibilité_fine = float(st.checkbox("Symptomes: Perte de la Sensibilité Fine"))
                S_trouble_de_language = float(st.checkbox("Symptomes: trouble de language"))
                S_vision_double = float(st.checkbox("Symptomes: Vision Double"))
                S_paresthésis = float(st.checkbox("Symptomes: Paresthésis"))
                CR_IRM_lésion_démyélinisantes_sus_tentoriel = float(st.checkbox("Compt Rendu IRM: Lésion Démyélinisantes sus Tentoriel"))
                CR_IRM_lésions_démyélinisantes_du_cordon_médullaire = float(st.checkbox("Compt Rendu IRM: Lésions Démyélinisantes du Cordon Médullaire"))
        with col2:
                EX_NEURO_synrome_pyramidale_4_mmbr_àl_hémicap_droit = float(st.checkbox("Examen Neurologique: Syndrome Pyramidale 4 mmbr à l'Hémicap Droit"))
